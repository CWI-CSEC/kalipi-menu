...
def upstatus(status):
    with open('/root/bin/kalipi-menu/status.txt','r') as rf: #Read the file and make changes
        contents = rf.read()
    ...
    with open('/root/bin/kalipi-menu/status.txt','w+') as wf:
        wf.write(contents)
    return 0


def chkstatus(status):
    with open('/root/bin/kalipi-menu/status.txt','r') as cf: #Read the file and make changes
        contents = cf.read()
